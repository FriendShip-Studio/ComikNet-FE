  ...
  <main class="main" :style="{ width: $route.meta.expand ? '100%' : '1200px' }">
    <div class="content">
      <div class="content-title">
        {{
          albumInfo?.name ? albumInfo.name + " - " + relativeCID : "漫画阅读"
        }}
      </div>
      ...
      <div class="content-body">
        <div class="settings-container">
          <a-affix :offset-top="110">
            <a-button
              ...
              shape="circle"
              size="large"
              @click="setShowReadingPanel(!showReadingPanel)"
              v-if="!isLoading"
        ...
        <div class="image-container" v-if="!isLoading">
          <JMImage
            v-for="image in imgList?.img_list"
            :key="image"
      ...
      <a-affix :offset-bottom="0" @change="setIsScrolling(!isScrolling)">
        <div class="chapter-panel" v-if="!isLoading">
          <a-button
            v-if="Number(relativeCID) > 1"
            key="prev"
            @click="router.push(`/comic/${albumID}/${Number(relativeCID) - 1}`)"
            type="primary"
          >
          ...
          <a-button
            ...
            class="return-btn"
            key="return"
            @click="$router.push(`/album/${albumID}`)"
            >返回章节目录</a-button
          >
          <span v-else class="return-btn" />
          <a-button
            v-if="Number(relativeCID) < chapterInfo?.length"
            key="next"
            @click="router.push(`/comic/${albumID}/${Number(relativeCID) + 1}`)"
            type="primary"
          >
      ...
      <a-drawer
        :visible="showReadingPanel"
        placement="right"
        @close="setShowReadingPanel(false)"
      >
        <template #title>
          <setting-outlined />
          阅读设置
        ...
        <div>
          ...
          <a-row class="drawer-inner-items">
            <a-col :span="12">
              <a-slider
                v-model:value="readingPage"
                :min="1"
                :max="imgList?.img_list.length"
          ...
          <a-button
            class="drawer-inner-items"
            type="primary"
            :href="anchorHref"
            @click="message.info(`已跳转至第${readingPage}页`)"
            >跳转</a-button
          >
        </div>
...
const getChapterImgs = async () => {
  ...
  try {
    ...
    const cInfo = chapterInfo.value.find(
      (item) => item.id === relativeCID.value
    );
