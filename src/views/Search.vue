            ...
            <a-spin :spinning="isLoading" tip="正在加载...">
                <div v-if="isSearched && !isLoading">
                    ...
                    <div class="page-panel" v-if="resultTotal && resultTotal > 80">
                        <a-pagination :current="Number(page)" simple :default-current="1"
                            :total="Math.ceil(resultTotal / 80) * 10" :min="1" @change="pageChanged" />
                    </div>
                    <a-back-top />
                </div>
            </a-spin>
...
import { OrderedListOutlined, HistoryOutlined, EyeOutlined, PictureOutlined, HeartOutlined } from "@ant-design/icons-vue";
...
const getResult = async () => {
    ...
    if (sortBy.value != "mr" && sortBy.value != "mv" && sortBy.value != "mp" && sortBy.value != "tf") {
        sortBy.value = "mr";
    }
...
watch(
    ...
    (s) => {
        try {
            if (s && typeof s == "string" && (s == "mr" || s == "mv" || s == "mp" || s == "tf")) {
                router.push({
                    query: {
                        query: query.value,
                        page: page.value,
